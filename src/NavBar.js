...
function NavBar({setIsAuth}){
  ...
  const libraryLink=(isAuth)? <Link to='/Library' className="menuItem">Library</Link>: <Link to='/Login'>Library</Link>;
  ...
  const loginStatement = <div className="logStat">{(isAuth)? `${username}` :  "Logged out"}</div>;
  const loginButton = 
